...
export default function CommunityPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF7FA] pt-20 pb-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        {/* 标题 */}
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-slate-900 mb-2">
            {isZh ? 'WtW 穿搭社区' : 'WtW Style Community'}
          </h1>
          <p className="text-slate-500 text-base">
            {isZh ? '发现灵感，分享你的 AI 穿搭' : 'Discover inspiration, share your AI try-on looks'}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-slate-400 text-lg">
              {isZh ? '还没有穿搭分享，成为第一个吧！' : 'No outfits shared yet. Be the first!'}
            </p>
            <button
              onClick={() => router.push('/tryon')}
              className="mt-4 px-6 py-2.5 bg-indigo-600 text-white rounded-full text-sm font-medium hover:bg-indigo-700 transition-colors"
            >
              {isZh ? '去试衣' : 'Try On Now'}
            </button>
          </div>
