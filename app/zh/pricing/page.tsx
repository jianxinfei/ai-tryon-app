      ...
      <header className="pt-8 sm:pt-12 pb-6 sm:pb-8 text-center px-4">
        ...
        <p className="mt-2 sm:mt-3 text-sm sm:text-base text-slate-500 max-w-xs sm:max-w-md mx-auto">
          {showReturningProducts
            ? '欢迎回来！选择积分包继续试穿'
            : '首次购买可获得奖励积分。慢慢选择适合您的方案。'}
        </p>
        {!currentUser && (
          <p className="mt-2 text-xs text-amber-600">
            未登录用户默认显示新用户价格。登录后根据购买记录自动切换。
          </p>
        )}
        {checkingUser && (
          <p className="mt-2 text-xs text-slate-400">正在检查用户状态...</p>
        )}
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 pb-12 sm:pb-20">
              ...
              >
                {loadingId === pack.productId ? (
                  <span className="flex items-center justify-center gap-1.5 sm:gap-2">
                    ...
                    <span className="truncate">正在跳转到支付...</span>
                  </span>
                ) : isAnyLoading ? (
                  '请稍候...'
                ) : (
            ...
            >
              ...
              <div className="mt-3 sm:mt-4 py-2 sm:py-3 border-t border-slate-100">
                <p className="text-center text-xl sm:text-2xl font-bold text-amber-600">
                  ...
                  <span className="text-xs sm:text-sm font-normal text-slate-500 ml-1">/月</span>
                </p>
        ...
        <div className="text-center space-y-3 mt-12 sm:mt-16">
          ...
          <p className="text-xs text-slate-400 pt-2">
            ...
            <a href="/terms" className="text-indigo-500 hover:text-indigo-600">服务条款</a> ·{' '}
            <a href="/privacy" className="text-indigo-500 hover:text-indigo-600">隐私政策</a>
          </p>
        </div>
      </main>
    </div>
